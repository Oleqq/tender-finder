...
| Поле | Значение |
|---|---|
| Текущий этап | Этап 1: Telegram и онбординг (планирование) |
| Статус MVP | Базовая инфраструктура готова; этап 1 ожидает внешние данные, для реальных миграций локальной PostgreSQL нужны учётные данные |
...
| WEB-01 | Реализовать React-каркас Mini App и проверку Telegram `initData` | Этап 1 | BLOCKED | Mini App безопасно открывается из бота | Telegram Bot Token, публичный HTTPS-домен и тестовый чат |
| BOT-01 | Подключить Telegram webhook и `/start`, `/help` | Этап 1 | BLOCKED | Бот открывает Mini App и отвечает в тестовом чате | Telegram Bot Token, публичный HTTPS-домен и тестовый чат |
