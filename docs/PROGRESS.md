...
| Статус MVP | Код этапа 0 готов; Vercel production-контур настраивается. Этап 1 ждёт юридические ссылки и production-хранилища, для реальных миграций локальной PostgreSQL нужны учётные данные |
...
| INF-03 | Подготовить Vercel production-контур | Переход к этапу 1 | IN PROGRESS | Production `GET /health` возвращает ожидаемый JSON | Runtime-конфигурация Laravel на Vercel стабилизируется; нужны managed PostgreSQL и Redis для постоянной работы |
...
- Результат: личное и исходное Git-хранилища синхронизированы при push, Vercel использует личное хранилище; production-переменные хранятся вне Git, а для необработанных Laravel-исключений добавлено безопасное сообщение в системный log stream без заголовков запроса и секретов.
- Текущая проверка: deployment Vercel создаётся успешно, но публичный `/health` пока возвращает `500`; диагностика runtime продолжается.
- Следующее действие: завершить диагностику `GET /health`, затем подключить managed PostgreSQL и Redis. После этого можно переходить к серверной проверке Telegram `initData`, webhook и экрану онбординга.
